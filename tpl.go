...
func (*DuduTpl) MapGet(arg1 interface{}, arg2 ...interface{}) (interface{}, error) {
	...
	if arg1Type.Kind() == reflect.Map && len(arg2) > 0 {
		...
		if storedVal.IsValid() {
			...
			if len(arg2) > 1 {
				return Tpl.MapGet(result, arg2[1:]...)
			}
			return result, nil
		}
		return nil, nil
...
func (*DuduTpl) HighLight(content string) string {
	//替换掉@用户标签
	reg := regexp.MustCompile("@([\\w\\x{2e80}-\\x{9fff}\\-]+)")
	str := reg.ReplaceAllStringFunc(content,
		func (name string) string {
			return "<a href=\"javascript:void(0)\" style=\"color:#F39D01\">" + name + "</a>"
...
func (*DuduTpl) HtmlFilter(content string) string {
	...
	str := re.ReplaceAllStringFunc(content, strings.ToLower)
	//去除STYLE
	...
	//过滤敏感词
	//需要加载三方包：  github.com/importcjj/sensitive
	//filter := sensitive.New()
	//filter.LoadWordDict("../dict/dict.txt")
	//filter.AddWord("习近平")
	//
	//str = filter.Replace(str, 42)
	return str
}